...
def workload(protoFile, modelFile):
    ...
    for l in cnn._layer_names:
        ...
        layerType =  cnn.layers[layerId].type
        # if (layerType == 'Data' or layerType == 'Input' or l == 'inputdata'):
        ...
        if (layerType == 'Convolution'):
            ...
            B = blobs[l].data.shape[0]
            U = blobs[l].data.shape[2]
            V = blobs[l].data.shape[3]
            nonZeros = np.count_nonzero(params[l][0].data)
            # Convolution Workload
            thisLayerWorkload = B*U*U*nonZeros #~ thisLayerWorkload = U*U*K*K*N*C
            print(l + " : " + str(blobs[l].data.shape) + " : ", human_format(thisLayerWorkload) + "MACs")
